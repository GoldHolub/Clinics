...
export class ClinicsRepository {
    ...
    async searchClinics(params) {
        try {
            ...
            const conditions = [];
            if (city)
                conditions.push(eq(Clinic.city, city));
            if (state)
                conditions.push(eq(Clinic.state, state));
            if (zip)
                conditions.push(eq(Clinic.postcode, zip));
            if (clinicName)
                conditions.push(like(Clinic.clinicName, `%${clinicName}%`));
            if (suburb)
                conditions.push(eq(Clinic.suburb, suburb));
            const query = db
                .select()
                .from(Clinic)
...
//# sourceMappingURL=ClinicsRepository.js.map